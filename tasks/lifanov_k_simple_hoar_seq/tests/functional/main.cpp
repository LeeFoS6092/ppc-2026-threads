...
static std::vector<int> GetRandomVector(size_t size) {
    ...
    std::mt19937 gen(rd());
    for (size_t i = 0; i < size; ++i) vec[i] = static_cast<int>(gen() % 1000);
    return vec;
}
...
const auto kTestTasksList = ppc::util::AddFuncTask<LifanovKSimpleHoarSEQ, InType>(kTestParam, "lifanov_k_simple_hoar_seq");
